...
fn overflowing_add(a: u64, b: u64, carry: bool) -> (u64, bool) {
    ...
    if sum >= a {
        ...
        if carry {
            (overflowing_add(sum, 1, carry).0, false)
        } else {
            (sum, false)
        }
    } else {
...
impl ops::Add<BigInt> for BigInt {
    ...
    fn add(self, rhs: BigInt) -> Self::Output {
        ...
        for i in 0..max_len {
            ...
            // for later.
            let (sum, carry) = overflowing_add(lhs_val, rhs_val, carry);
            result_vec.push(sum);
        }
        // **Exercise 08.2**: Handle the final `carry`, and return the sum.
        if carry {
            let (last_digit, fin_carry) = overflowing_add(result_vec[max_len], 1, false);
            result_vec[max_len] = last_digit;
            if fin_carry {
                result_vec.push(1);
                return BigInt::from_vec(result_vec);
            } else {
                return BigInt::from_vec(result_vec);
            }
        }
